...
class ConfigService:
                ...
                "models": [
                    {
                        ...
                        "display_name": "Gemini 3.5 Flash - 最新极速",
                    ...
                    {
                        "name": "gemini-3.1-pro",
                        "display_name": "Gemini 3.1 Pro - 最新旗舰",
                    ...
                    {
                        "name": "gemini-2.5-pro",
                        "display_name": "Gemini 2.5 Pro - 2.5代旗舰",
                        "input_price_per_1k": 0.00125,
                        "output_price_per_1k": 0.005,
                        "context_length": 1000000,
                        "currency": "USD"
                    },
                    {
                        ...
                        "display_name": "Gemini 2.5 Flash - 2.5代快速",
                        "input_price_per_1k": 0.000075,
                        "output_price_per_1k": 0.0003,
                        "context_length": 1000000,
                        "currency": "USD"
    ...
    async def init_default_llm_providers(self) -> bool:
        """初始化默认大模型厂家数据"""
        try:
            db = await self._get_db()
            providers_collection = db.llm_providers
            count = await providers_collection.count_documents({})
            if count > 0:
                return True
            ...
            for item in providers_data:
                item["created_at"] = now
                item["updated_at"] = now
                await providers_collection.insert_one(item)
            logger.info(f"✅ 自动初始化了 {len(providers_data)} 个默认大模型厂家数据")
            return True
        except Exception as e:
            logger.error(f"❌ 初始化默认大模型厂家数据失败: {e}", exc_info=True)
            return False
    ...
    async def get_llm_providers(self) -> List[LLMProvider]:
        ...
        try:
            ...
            if not providers_data:
                logger.info("⚠️ 数据库中无 LLM 厂家数据，自动初始化默认厂家...")
                await self.init_default_llm_providers()
                providers_data = await providers_collection.find().to_list(length=None)
